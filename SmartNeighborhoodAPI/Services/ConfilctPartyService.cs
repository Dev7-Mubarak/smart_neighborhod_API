...
{
    public class ConfilctPartieservice
    {
        private readonly ApplicationDbContext _context;
        readonly IMapper _mapper;

        public ConfilctPartieservice(ApplicationDbContext context, IMapper mapper)
        {
            _context = context;
            _mapper = mapper;
        ...
        {
            var ConfilctParties = await _context.ConfilctParties.AsNoTracking().ToListAsync();
            if (ConfilctParties.Count > 0)
            {
                var ConfilctPartyDtos = _mapper.Map<IEnumerable<ConfilctPartyDto>>(ConfilctParties);
                return ApiResponse<IEnumerable<ConfilctPartyDto>>.Success(ConfilctPartyDtos);
            }

            return ApiResponse<IEnumerable<ConfilctPartyDto>>.Error(HttpStatusCode.NotFound, "No ConfilctParties Found");
